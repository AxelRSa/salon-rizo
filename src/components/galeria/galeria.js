...
const initializator = async () => {
	...
	display_none(slider_arrow_left);
	display_flex(slider_arrow_right);
	// reset
	...
	slider_arrow_right.onclick = () => {
		...
		display_flex(slider_arrow_left)
		...
		if (slider_position == slider_index.length - 1) {
			display_none(slider_arrow_right)
		}
...
const index_slider_generator = (packs_quantity) => {
	...
	let index = []
	const left_size = document.body.clientWidth
...
const display_none = (element) => {
	element.style.display = "none"
}

const display_flex = (element) => {
