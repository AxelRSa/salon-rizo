...
export const render_modal = (publications, image) => {
	...
	if (index_modal == 0) {
		modal_arrow_left.style.visibility = "hidden"
	} else if (index_modal == publications.length - 1) {
		modal_arrow_right.style.visibility = "hidden"
	} else {
		modal_arrow_left.style.visibility = "visible"
		modal_arrow_right.style.visibility = "visible"
	}

	modal_arrow_left.onclick = () => {
		index_modal--
		render_modal(publications, publications[index_modal])
	}
...
const put_dots = (image) => {
	const dots_quantity = image.children.length
